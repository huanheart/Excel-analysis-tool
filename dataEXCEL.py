...
#     else:
#         global temproot
#         writer = pd.ExcelWriter(temproot + '/mycell.xls',engine='openpyxl')
...
def SaveExcel(df, isChecked):
    # 定义文件路径
    if isChecked:
        file_path = 'mycell.xlsx'
    else:
        global temproot
        file_path = temproot + '/mycell.xls'

    # 如果文件已经存在，删除它
    if os.path.exists(file_path):
        os.remove(file_path)

    # 创建 ExcelWriter 对象并保存数据
...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def click1(self):
        # 文件夹路径
        global root
        #注意这里是选择文件夹，是不能找到文件的，即不能指定文件，只能指定文件夹下面的所有文件
